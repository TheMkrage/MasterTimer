...
extension NSTimeInterval {
    ...
    func secondsToHoursMinutesSecondsString() -> NSString {
        let tuple = self.secondsToHoursMinutesSeconds()
        var runningString = ""
...
class MainMenuViewController: UITableViewController {
    ...
    func loadSkills() {
        ...
        if let curSkills = defaults.valueForKey("Skills") as? NSArray {
            for skill in curSkills {
                let s:Skill = Skill(skillName: skill.objectForKey("Name") as! String, timeInDouble: skill.objectForKey("Time") as! Double)
                self.skills.addObject(s)
            }
        }else { // initial launch, make array here
            let defaults = NSUserDefaults.standardUserDefaults()
            let newSkills: NSMutableArray = NSMutableArray()
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let curSkill = self.skills.objectAtIndex(indexPath.row) as! Skill
